  ...
  const excelData = useCallback(() => {
    ...
    Object.keys(projects).forEach(projectId => {
      ...
      result[projectId] = {
        ...
        utilidadEstimadaFacturado: formatMoney(project.utilidadEstimadaConFactura),
        utilidadRealFacturado: formatMoney(project.utilidadRealConFactura),
        ...
        totalEgresosProyecto: formatMoney(project.totalEgresosProyecto || project.totalSaldoPorPagarProveedores),
        balanceDelPresupuesto: formatMoney(project.balanceDelPresupuesto),
        
        // 📅 TOTAL DE COBRANZAS (suma de la nueva funcionalidad)
        totalCobranzas: project.cobranzas ? 
          formatMoney(project.cobranzas.reduce((sum, c) => sum + (parseFloat(c.monto) || 0), 0)) : 
          'S/ 0.00'
      };
    });
    return result;
